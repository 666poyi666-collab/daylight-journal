...
1. 先让 AI 复述需求边界和当前代码事实。
2. 先做最小可验证改动，不默认引入 agent、复杂 skill 或额外 MCP。
3. 每一步都留下可读日志：请求、响应状态、关键状态变化和失败原因。
4. 先修根因，再做视觉或架构扩展；不要用临时桥接掩盖同步问题。
5. 完成后执行构建、接口检查和目标设备验收，并记录结果。
